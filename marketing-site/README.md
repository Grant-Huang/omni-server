...
| `styles.css` | 共享样式（颜色/字体 token 与 `omni` 仓库的 `app/` 保持一致，不要在这里发明新颜色） |
...
- 下载页的「打开 Web App」按钮目前指向 `https://app.jia.family` 占位域名——这是产品实际部署（`omni` 仓库的 `app/` 部署上线之后）要绑定的域名，暂时还没有真实内容，上线前需要替换成真实地址。
